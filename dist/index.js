...
const formatTimes = timeLeft => Object.assign(...Object.entries(timeLeft).map(([k, v]) => ({
  [k]: v.toFixed(0).padStart(2, "0")
})));
const secondsBetweenTwoDates = (a, b) => (Date.parse(a) - Date.parse(b)) / 1000;
const secondsToTimeleft = seconds => {
  const timeLeft = {
    hours: 0,
    minutes: 0,
    seconds: 0
